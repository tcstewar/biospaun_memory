...
class BioSpaunMemory(ctn_benchmark.Benchmark):
    ...
    def evaluate(self, p, sim, plt):
        ...
        if len(nn_interest_post) > 0:
            ...
            nn = nn_interest_post[0]
            enc = encoder_data[self.get_dataset_name('pre', p.drug_name)
                               ][nn, :]
            plt.subplot(1, 2, 1)
            plt.plot(sim.trange(),
                     smoothed_data_pre[:, nn],color='b',linewidth=2.0, label='Control')
            plt.plot(sim.trange(),
                     smoothed_data_post[:, nn],color='r',linewidth=2.0, label='%s' % p.drug_name)
            plt.fill_between([0,1],[0,0],[float(plt.ylim()[1]),float(plt.ylim()[1])], color='gray')
            plt.legend(loc='lower right')
            label_xticks = ['Fix Cue','Delay Period','2','4','6','8']
